...
var query = function() {
	...
	var url = path_ + "/view/shop/comment/comment!queryCommentList.action";
...
var showEditCommentWin = function(id) {
	...
	$.ajax({
		type : "POST",
		url : path_ + "/view/shop/comment/comment!queryCommentById.action",
		data : data,
		dataType : "json",
		success : function(data) {
			setCommentInfo(data);
...
var delComment = function(id) {
	showConfirm("确认删除？", function() {
		...
		$.ajax({
			type : "POST",
			url : path_ + "/view/shop/comment/comment!delComment.action",
			data : data,
			dataType : "json",
