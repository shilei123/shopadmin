...
function initCategoryTree() {
	$.ajax( {
		type : "POST",
		url : path_ + "/view/shop/admin/catePropPropval/catePropPropval!queryCategoryTree.action",
		dataType : "json",
...
var categoryClick = function(node) {
	...
	$.ajax({
		type : "POST",
		url : path_ + "/view/shop/admin/catePropPropval/catePropPropval!queryListByCateId.action",
		dataType : "json",
...
var checkPropPropVal = function(listCheck, propPropvalId){
	for(x in listCheck){
		if(listCheck[x].ppvId==propPropvalId){
			return true;
		}
	};
	return false;
...
var saveIt = function(cateId) {
	...
	$.ajax({
		type : "POST",
		url : path_ + "/view/shop/admin/catePropPropval/catePropPropval!saveCatePropPropVal.action",
		data : data,
		dataType : "json",
		success : function(json) {
			showAlert("操作成功");
