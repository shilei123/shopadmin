...
var query = function() {
	...
	var url = path_ + "/view/propValue/propValue!query.action";
...
$("#saveBtn").click(function() {
	...
	if(propValueId!=null /*&& propValueId!=undefine*/ && propValueId!=""){
		url = path_ + "/view/propValue/propValue!updatePropValue.action"
	}else{
		url = path_ + "/view/propValue/propValue!addPropValue.action"
	}
...
var showEditPropWin = function(id) {
	...
	$.ajax({
		type : "POST",
		url : path_ + "/view/propValue/propValue!queryPropValueById.action",
		data : data,
		dataType : "json",
...
var deleteProp = function(id) {
	showConfirm("确认删除？", function() {
		...
		$.ajax({
			type : "POST",
			url : path_ + "/view/propValue/propValue!delete.action",
			data : data,
			dataType : "json",
			success : function(json) {
				showAlert("操作成功");
