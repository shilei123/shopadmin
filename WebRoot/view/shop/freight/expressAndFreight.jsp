    	...
    	<div class="admin-content-body">
      		<div class="am-g">
      			<form id="form1" method="post">
      				<input type="hidden" name="queryParams.freightId" id="freightId" value="${freightId }"/>
      				<input type="hidden" name="queryParams.checkExpressIds" id="checkPropIds" value=""/>
      				<div style="width: 100%; height:295px; overflow: auto;">
		            <table class="am-table am-table-bordered am-table-striped am-table-hover" id="freightParamsTable">
		              <thead>
			              <tr>
			                <th field="index">序号</th>
			                <th field="checkbox">选择</th>
							<th field="forShort">快递服务商</th>
			              </tr>
		              </thead>
		            </table>
		            </div>
...
<script type="text/javascript">
	$(function() {
		queryExpAndFreightCheck();
	});
	var queryExpAndFreightCheck = function(){
		$.ajax({
			url : "${path }/view/shop/freight/freight!queryFreAndExpCheck.action",
			type : 'POST',
	...
	var writeHidden = function(data){
		var checkExpressIds = "";
		for(i in data.freightList){
			checkExpressIds += data.freightList[i].expressId;
			checkExpressIds += ",";
		};
		$("#checkPropIds").val(checkExpressIds);
	};
	
	var query = function() {
		var data = formGetCurrentJsp("form1");
		var url = "${path }/view/shop/freight/freight!queryFreAndExpList.action";
		pageDataCurrentJsp(url, "freightParamsTable", data);
	};
	...
	function formGetCurrentJsp(formId){
		...
		return obj;freightListfreightList
	}
	...
	var closeCurrentWin = function() {
		window.parent.closeModal("doc-modal-1");
	};
