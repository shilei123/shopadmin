...
@Repository("scImageDAO")
public class ScImageDAO extends PageDAO {
	/**
	 * 根据名称查询
	 * @param pageBean
	 * @return pageData
	 */
	public final String SELECT_SQL = " select t.id,t.img_name,t.file_name,t.img_path,t.img_type,t.img_size,to_char(t.create_time,'yyyy-mm-dd hh24:mi:ss') create_time from sc_image t where flag=? ";
	
	@SuppressWarnings("unchecked")
	public List<ScImage> queryImagesPagination(PageBean pageBean) {
		List<String> params = new ArrayList<String>();
		params.add(FlagEnum.ACT.getCode());
		String sql = this.buildWhereSql(pageBean, params);
		List<ScImage> pageData = this.query(sql, params, DBUtil.getInstance(), pageBean);
		return pageData;
	}
	...
	private String buildWhereSql(PageBean pageBean, List<String> params) {
		...
		if (pageBean.getQueryParams() != null && !pageBean.getQueryParams().isEmpty()) {
			...
			if (StringUtils.isNotBlank(imgName)){
				params.add("%"+imgName+"%");
				sql.append(" and t.img_name like ? ");
			}
